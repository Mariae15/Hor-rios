...
firebase.firestore()
  ...
  .then((doc) => {
    if (doc.exists) {
      ...
      for (const celulaId in dados) {
        const celula = document.getElementById(celulaId);
        if (celula) {
          const aula = dados[celulaId];
            if (aula && aula.materia && aula.nome) {
              celula.innerHTML = `<strong>${aula.materia}</strong><br><small>${aula.nome}</small>`;
            }
...
firebase.firestore()
  ...
  let celulaOrigem = null;

function arrastar(event) {
  celulaOrigem = event.target;
